...
int		findMax(dblist *list, int need)
{
    element		*tmp = list->first;
	int			sum  = 0;
    ...
    while(tmp)
    {
		if (tmp->size >= need && tmp->size < 3572786) {
			sum = tmp->size;
		}
        tmp = tmp->next;
    }
...
int		main (int ac, char **av)
{
	...
	init(&dirList);
	if (!(indexStr = malloc(sizeof(char*) * 1000)))
		return (0);
    file = fopen(av[1], "r+");
    if (file != NULL)
    {
		while (fgets(buf, BUFFER_SIZE, file) != NULL)
		{
			if (strstr(buf, "cd ") != NULL) { // cd + alpha
				if (buf[5] >= 97 && buf[5] <= 122) {
					if (doesExist(tabList, ft_cutString(buf, ' ')) <= 1) { // file name exist
						itoa(1, indexStr);
					...
					else { // file name doesnt exist
						// printf("[%d]\n", doesExist(tabList, ft_cutString(buf, ' ')));
			...
			else if (((strstr(buf, "dir ") != NULL))) { // dir inside dir
				if (doesExist(tabList, ft_cutString(buf, ' ')) <= 1) { // dir doesnt exist
					itoa(1, indexStr);
					tabList = addNewTab(tabList, strcat(ft_cutString(buf, ' '), indexStr));
					addDirectoriesInside(&dirList, currentDir, strcat(ft_cutString(buf, ' '), indexStr));
				}
				// else if (doesExist(tabList, ft_cutString(buf, ' ')) == 1) { // dir doesnt exist 1 time
				// 	itoa(doesExist(tabList, ft_cutString(buf, ' ')), indexStr);
				// 	tabList = addNewTab(tabList, strcat(ft_cutString(buf, ' '), indexStr));
				// 	addDirectoriesInside(&dirList, currentDir, strcat(ft_cutString(buf, ' '), indexStr));
				// }
				else { // dir doesnt exist 1 time
					itoa(doesExist(tabList, ft_cutString(buf, ' ')), indexStr);
					tabList = addNewTab(tabList, strcat(ft_cutString(buf, ' '), indexStr));
					addDirectoriesInside(&dirList, currentDir, strcat(ft_cutString(buf, ' '), indexStr));
				}
	...
	// view(dirList);
	// printf("=========\n");
	int	spaceLeft = 70000000 - 43296980;
	int	need = 30000000 - spaceLeft;
	printf("need = [%d]\n", need);
	printf("rep = [%d]", findMax(&dirList, need)); // total size - total space available;
	// 4183246
	// 38955413
	// manque dans mon total= 4882370; -> total = 43 837 783
    return 0;
}
